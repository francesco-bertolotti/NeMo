...
from nemo.collections.llm.gpt.model import NemotronConfig, NemotronModel
...
@dataclass
class Nemotron4ConfigItalia10B(NemotronConfig):
    """
...
def model() -> run.Config[pl.LightningModule]:
    """
    Create a model configuration for the Nemotron4 15B model.
    """
    italia10B_config = run.Config(Nemotron4ConfigItalia10B)
    return run.Config(NemotronModel, config=italia10B_config)
...
def trainer(
    ...
    strategy = run.Config(
        ...
        ckpt_load_strictness=False,  # TODO: set to True once checkpoint loading is fixed
        ddp=run.Config(
